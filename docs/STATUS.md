...
M9 완료를 막지 않아 남긴 것들이다. 착수할 때 이 항목을 지운다.

- **엔진 진단이 실행 오류에서 영문 원문으로 노출된다.** `ui/src/labels.ts`의
  `diagnosticHelp`는 도움말 탭의 코드 목록에서만 쓰이고, 실행 실패 경로
  (`ResultPanel`의 `실행 오류: ...` 메시지)에는 연결돼 있지 않다. `W009`/`E010`처럼
  로컬 검증기가 아니라 엔진에서만 나오는 코드는 사용자가 영문 문자열을 그대로 본다.
  실행 오류 문자열에서 진단 코드를 추출해 `diagnosticHelp`로 매핑할 것.
- **`validateLocally`가 accumulator 사전계산 테이블 크기를 모델링하지 않는다.**
  실시간 검증 패널은 순수 TS이므로 `W009`/`E010`을 실행 전에 예고하지 못하고,
  사용자는 실행해야 알 수 있다. `E010`이 56ms 만에 거부하므로 실질 피해는 없지만
  "블록을 놓는 즉시 갱신되는 검증 패널"(§10.2)의 커버리지 공백이다.
  `ui/src/validator.ts`에 `제어상태 × 시행 × 리프 × (max+1)` 추정을 추가하면
  `max` 입력 중에 바로 경고할 수 있다.
- **10,000,000 엔트리 초과 시 `W009`와 `E010`이 함께 발생한다.** 단계적 신호로 보면
  무해하지만 중복이다. 정리한다면 `E010` 발생 시 `W009`를 생략한다.
...
- 폰의 Blockly 편집 정책은 소유자 결정 전까지 기존 동작을 유지한다. Blockly 지연
  로딩과 Vite React 플러그인 설정도 선택 작업으로 남겼다.
