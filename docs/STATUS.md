...
- MC↔DP 100만 회 Wilson 교차검증, exact↔ScaledF64 극소확률 비교, 지급 전파 회귀 테스트
...
## 2026-07-27 DESIGN §13.3 핵심 검증

- MC 1,000,000회와 DP 결합분포의 Wilson 95% 교차검증 추가
- ExactInt와 ScaledF64를 `N=200, p=0.007` 전 셀에서 상대오차 `≤ 1e-10`으로 비교하며, `f64` 언더플로 셀도 유리수로 검증
- 확정 픽업 지급 후 파생 `nStar3` 분포의 정확한 +1 이동 검증
- 교차검증 과정에서 MC alias table 마지막 버킷 유실과 Wilson 경계 반올림 버그를 발견·수정하고 직접 회귀 테스트 추가
- GitHub Actions CI 실행 성공: Rust 13/13, UI 4/4
- 다음 우선순위: 지급 의미론 4조합, 기하/음이항 해석해, 프리셋 골든 파일, IR 퍼징
